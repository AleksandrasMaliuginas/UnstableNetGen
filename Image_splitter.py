...
def split_image(image_path,out_path='', tile_size=1900):
    try:
    ...
    if lower-upper<100:
        lower+=100
    tile = image.crop((left, upper, right, lower))
    tile_path = f"tile_{num_tiles_horizontal-1}_{num_tiles_vertical}.jpg"
    tile_path = os.path.join(out_path, tile_path)
    tile.save(tile_path)
    tiles.append(tile_path)
    ...
    tile = image.crop((left, upper, right, lower))
    tile_path = f"tile_{num_tiles_horizontal}_{num_tiles_vertical-1}.jpg"
    tile_path = os.path.join(out_path, tile_path)
    tile.save(tile_path)
    tiles.append(tile_path)
    
    
    #num_tiles_horizontal+=1
    #num_tiles_vertical+=1
